...
function pingServ (serverRegistration, timeout, version) {
  return new Promise(async (resolve, reject) => {
    switch (serverRegistration.data.type) {
      case 'PC':
        serverRegistration.dnsResolver.resolvePC((host, port, remainingTimeout) => {
      ...
      case 'PE':
        ...
        /*
        (node:9959) MaxListenersExceededWarning: Possible EventEmitter memory leak detected. 11 uncaughtException listeners added to [process]. Use emitter.setMaxListeners() to increase limit (Use `node --trace-warn)
        */
        process.on('uncaughtException', function (err) {
          if((err && (err.code === 'ENOTFOUND' || err.code === 'ENODATA'))) {
            if (con != null) {
              con.close()
              return reject(new Error("Bad server name"))
            }
          }
        });
        con = new RakClient({
          useRaknetWorker: true,
          host: serverRegistration.data.ip,
