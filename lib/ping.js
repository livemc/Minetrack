...
function ping (serverRegistration, timeout, callback, version) {
  switch (serverRegistration.data.type) {
    ...
    case 'PE':
      ...
      ping({ host: serverRegistration.data.ip, port: serverRegistration.data.port || 19132 }).then(res => {
        callback(null, {
          players: {
            online: capPlayerCount(serverRegistration.data.ip, parseInt(res.playersOnline))
          }
        })
      }).catch(reason => {
        callback(reason)
